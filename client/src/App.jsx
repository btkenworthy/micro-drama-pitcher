...
export default function App() {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      const res = await fetch(API, {
        ...
        body: JSON.stringify({ prompt, generateVideos: withVideo }),
      });
  ...
  const { script, images, video } = result || {};
  ...
  return (
    <div style={styles.app}>
      ...
      <form onSubmit={handleGenerate} style={styles.form}>
        <input
          style={styles.input}
          placeholder="Describe your micro drama idea..."
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          aria-label="Drama idea prompt"
        />
      ...
      {error && <div style={{ ...styles.card, borderLeft: "3px solid #e74c3c" }}>Error: {error}</div>}
        ...
        <>
          ...
          {script.scenes?.map((scene, i) => (
            <div key={i} style={styles.card}>
              ...
              {scene.dialogue?.map((d, j) => (
                <div key={j} style={styles.dialogue}>
                  <span style={styles.charName}>{d.character}:</span> {d.line}
                </div>
              ))}

              {images?.[i] && <img src={images[i]} alt={`Scene ${scene.sceneNumber}`} style={styles.sceneImg} />}
            </div>
          ))}
          ...
          {video && (
            <div style={styles.card}>
              <h3 style={{ color: "#ffd200" }}>🎥 Generated Video</h3>
              <video controls style={styles.video}><source src={video} /></video>
            </div>
          )}
        </>
      )}
